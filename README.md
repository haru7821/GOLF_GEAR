...
npm run lint
npm run check:data   # 아이언 모델 데이터 무결성 검사
```
...
src/
  ...
  data/models.ts          2020년 이후 아이언 모델 (로프트·구조·특징·출처)
...
- **확인되지 않은 수치는 추정해서 채우지 않는다.** 공백은 `specNote`에 명시하고
  화면에도 경고로 노출한다. 세트 안 로프트 간격이 규칙적이라 값을 짐작할 수
  있어도 비워 둔다.
- **자료끼리 값이 다르면 한쪽을 고르지 않는다.** 충돌 사실 자체를 적는다.
- `npm run check:data`가 모델 id 유일성, 브랜드 참조, 로프트 파싱·범위와
  **"클럽 번호가 올라가면 로프트도 커진다"는 불변식**을 검사한다. 이 검사는
  배포 워크플로에서도 돌아, 전사 오류가 있으면 빌드가 실패한다.
